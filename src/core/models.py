...
class Keyword(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    group_id: Mapped[UUID] = mapped_column(ForeignKey("keyword_groups.id", ondelete="CASCADE"))
    keyword: Mapped[str] = mapped_column(String(200), nullable=False)
    synonyms: Mapped[list[str] | None] = mapped_column(ARRAY(String))  # Alternative spellings
    weight: Mapped[float] = mapped_column(default=1.0)  # Importance weight
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, server_default=func.now())
...
class Source(Base):
    ...
    status: Mapped[SourceStatus] = mapped_column(String(50), default=SourceStatus.ACTIVE)
...
class Schedule(Base):
    ...
    keyword_group_ids: Mapped[list[str] | None] = mapped_column(ARRAY(UUID(as_uuid=True)))
...
class JobExecution(Base):
    ...
    id: Mapped[UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid4)
    schedule_id: Mapped[UUID | None] = mapped_column(ForeignKey("schedules.id", ondelete="SET NULL"))
...
class Content(Base):
    ...
    matched_keyword_groups: Mapped[list[str] | None] = mapped_column(ARRAY(UUID(as_uuid=True)))

    # Status
    status: Mapped[ContentStatus] = mapped_column(String(50), default=ContentStatus.NEW)
...
class NotificationConfig(Base):
    ...
    # Filtering
    keyword_group_ids: Mapped[list[str] | None] = mapped_column(ARRAY(UUID(as_uuid=True)))
    min_importance_score: Mapped[float] = mapped_column(default=0.0)
    min_relevance_score: Mapped[float] = mapped_column(default=0.0)
...
class NotificationLog(Base):
    ...
    content_id: Mapped[UUID] = mapped_column(ForeignKey("contents.id", ondelete="CASCADE"))
